...
(function (jaco) {
    ...
    var Jaco = (function () {
        ...
        */
        Jaco.prototype.substring = function (indexA, indexB) {
            var start = Math.min(indexA, indexB);
            var end = Math.max(indexA, indexB);
            if (this._includedSurrogate) {
        ...
        */
        Jaco.prototype.hasSurrogatePair = function () {
            return new RegExp("[" + jaco.HIGH_SURROGATE + "][" + jaco.LOW_SURROGATE + "]").test(this._str);
        };
        /**
        ...
        */
        Jaco.prototype.hasUnpairedSurrogate = function () {
            return new RegExp("[^" + jaco.HIGH_SURROGATE + "][" + jaco.LOW_SURROGATE + "]|[" + jaco.HIGH_SURROGATE + "][^" + jaco.LOW_SURROGATE + "]").test(this._str);
        };
        /**
        * ペアになっていないサロゲートを削除する
        *
        ...
        */
        Jaco.prototype.removeUnpairedSurrogate = function () {
            var h = jaco.HIGH_SURROGATE;
            var l = jaco.LOW_SURROGATE;
        ...
        * @return インスタンス自身
        */
        Jaco.prototype.toNarrowSymbol = function () {
            // スペースの変換
            this.replace(toPattern(jaco.SPACE_CHARS), ' ');
            // 半角英数記号の変換
            this._shift(toPattern(jaco.FULLWIDTH_SING_CHARS), -65248);
            return this;
        };
        /**
        * 記号を全角に変換
        ...
        * @return インスタンス自身
        */
        Jaco.prototype.toWideSymbol = function () {
            // スペースの変換
            this.replace(' ', '\u3000');
            // 半角英数記号の変換
            this._shift(toPattern(jaco.SIGN_CHARS), 65248);
            return this;
        };
        /**
        * 濁点を追加する
        ...
        */
        Jaco.prototype.convertIterationMarks = function () {
            var hira = jaco.HIRAGANA_CHARS_IGNORE_ITERATION_MARKS;
            var kata = jaco.KATAKANA_CHARS_IGNORE_ITERATION_MARKS;
            var kanaWithIterationMarks = new RegExp("([" + hira + kata + "])([\u309D\u309E\u30FD\u30FE])");
            var conv = function (_str) {
        ...
        * @return インスタンス自身
        */
        Jaco.prototype._update = function () {
            this._includedSurrogate = includedSurrogate(this._str);
            if (this._includedSurrogate) {
                this._array = this._str.match(new RegExp("[" + jaco.HIGH_SURROGATE + "][" + jaco.LOW_SURROGATE + "]|[^" + jaco.HIGH_SURROGATE + jaco.LOW_SURROGATE + "]", 'g')) || [];
            }
            else {
                this._array = null;
            }
    ...
    */
    function includedSurrogate(chars) {
        return new RegExp("[" + jaco.HIGH_SURROGATE + jaco.LOW_SURROGATE + "]").test(chars);
    }
})(jaco || (jaco = {}));
